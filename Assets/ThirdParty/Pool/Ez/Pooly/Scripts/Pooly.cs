...
namespace Ez.Pooly
{
    ...
    [AddComponentMenu("我的组件/对象池/必须放 1 个在场景中")]
    [DisallowMultipleComponent]
    public partial class Pooly : MonoBehaviour
    {
        private const string METHOD_NAME_ONSPAWNED = "OnSpawned";            // 传递下去 产生的名称
        private const string METHOD_NAME_ONDESPAWNED = "OnDespawned";        // 传递下去 消失的名称



        #region 私有
        public const string DEFAULT_CATEGORY_NAME = "No Category";    // 默认类别名称
        /// <summary>
        ...
        /*
         结构：   Pooly           ->  控制整个 对象池,不销毁        （ 简称 对象池管理者 ）
                  PoolyDespawner  ->  修改预制体参数与设置          （ 简称 预制体对象 ）
                  PoolySpawner    ->  修改产生地点 与 每个地点产生的时间间隔   （简称 发射地点）
                ...
                其中 -> 
                       Pooly   -> 有多个类别，1 个类别 对应 1个父对象
                       类别    -> 有多个 Item ( Bean 对象 )
                       Item    -> 带 1 个预制体、最多克隆数量、限制每帧创建数 的 Bean 对象
