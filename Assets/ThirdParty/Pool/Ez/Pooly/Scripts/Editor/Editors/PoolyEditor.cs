...
namespace Ez.Pooly
{
    ...
    public class PoolyEditor : QEditor
    {
        ...
        public static void DrawItemSettings(Pooly.Item item, float width, AnimBool aBool, IconColor iconColor, Color accentColor)
        {
            ...
            if (!item.allowRecycleClones)
            {
                QUI.BeginHorizontal(width);
                {
                    ...
                    QUI.ResetColors();
                    content.text = "开启：一开始克隆出来的数量不够时，可以继续增加";
            ...
            if (item.allowInstantiateMore)
            {
                QUI.BeginHorizontal(width - 22);
                {
                    ...
                    QUI.ResetColors();
                    content.text = item.limitCloneCount ? "限制一下克隆数的最大值  ->" : "要不要限制一下克隆数的最大值";
                    QUI.Label(content.text, Style.Text.Normal, QStyles.CalcSize(content, Style.Text.Normal).x * aBool.faded);
                    if (item.limitCloneCount)
                    {
                        QUI.SetGUIBackgroundColor(accentColor);
            ...
            if (!item.allowInstantiateMore)
            {
                QUI.BeginHorizontal(width);
                {
                    ...
                    QUI.ResetColors();
                    content.text = "克隆体不够时，是否使用最早的克隆体(即使还存活)";
                    QUI.Label(content.text, Style.Text.Normal, QStyles.CalcSize(content, Style.Text.Normal).x * aBool.faded);
                    QUI.FlexibleSpace();
                }
                QUI.EndHorizontal();
            }
            else if (item.allowRecycleClones)
            {
                item.allowRecycleClones = false;
            }
            ...
            QUI.BeginHorizontal(width - 22);
            {
                ...
                QUI.ResetColors();
                content.text = item.limitCloneCreationPerFrame?"要不要限制每帧创建的克隆数量": "限制每帧创建的克隆数量";
                QUI.Label(content.text, Style.Text.Normal, QStyles.CalcSize(content, Style.Text.Normal).x * aBool.faded);
                QUI.FlexibleSpace();
            }
            QUI.EndHorizontal();
            ...
            QUI.BeginHorizontal(width - 22);
            {
                ...
                QUI.ResetColors();
                content.text = item.debug?"开启：产生 和 消失都会 Log 一下": "产生 和 消失都会 Log 一下";
                QUI.Label(content.text, Style.Text.Normal, QStyles.CalcSize(content, Style.Text.Normal).x * aBool.faded);
                QUI.FlexibleSpace();
            }
            QUI.EndHorizontal();
        ...
        protected override void GenerateInfoMessages()
        {
            ...
            infoMessage.Add("SearchNoResults", new InfoMessage()
            {
                title = "您的搜索未返回任何结果", message = "", show = new AnimBool(false), type = InfoMessageType.Info
            });
            infoMessage.Add("PoolIsEmpty", new InfoMessage()
            {
                title = "对象池是空的", show = new AnimBool(false, Repaint), type = InfoMessageType.Info
            });
        }
        ...
        private void DrawEditorJie()                             // 没运行，在编辑器的界面
        {
            ...
            QUI.BeginHorizontal(WIDTH_420);
            {
                ...
                QUI.Toggle(debug);
                QUI.Label("产生/消失 是否 Debug下", Style.Text.Normal, 120);
            }
            QUI.EndHorizontal();
            QUI.ResetColors();
            ...
            QUI.BeginHorizontal(WIDTH_420);
            {
                ...
                QUI.ResetColors();
                QUI.Label("自动添加缺失的 items", Style.Text.Normal, 130);
                QUI.BeginVertical(WIDTH_420 - 130);
                {
                    QUI.Space(-17);
                    QUI.Label("拖 预制体 到这", DropZoneBoxStyle, WIDTH_420 - 150, 32);
                    ...
                    if (Event.current.type == EventType.DragPerform)
                    {
                        if (dropZone.Contains(Event.current.mousePosition))
                        {
                            ...
                            foreach (var obj in DragAndDrop.objectReferences)
                            {
                                ...
                                {
                                    QUI.DisplayDialog("不能添加新 Item 到对象池中!",
                                                                "这个 '" + obj.name + "' 您尝试添加到池中的对象不是gameObject",
                                                                "Ok");
                                }
                                else if (PrefabUtility.GetPrefabObject(obj) == null) // If TRUE then this is not a prefab
                                {
                                    QUI.DisplayDialog("不能添加新 Item 到对象池中!",
                                                                "这个 '" + obj.name + "' 您尝试添加到池中的 gameObject 不是预制体" +
                                                                "\n\n" +
                                                                "您不能也不应该尝试添加场景对象",
                                                                "Ok");
                                }
        ...
        private void DrawRuntimeEditor(float width)              // 运行时的界面
        {
            ...
            QUI.Label("自动添加缺失的 Items   -> " + (pooly.autoAddMissingItems ? "启用" : "禁用"), Style.Text.Help);
            QUI.Space(SPACE_2);
        ...
        void DrawNewItemArea()
        {
            QUI.BeginHorizontal(WIDTH_420);
            {
                ...
                if(QUI.GhostButton("创建一个新的 Item", QColors.Color.Green, WIDTH_140))
                {
                    if(categoriesList[newItemCategoryListIndex].Equals(NEW_CATEGORY))
                    {
                        if(string.IsNullOrEmpty(newItemCategoryName))
                ...
                {
                    QUI.Space(-1);
                    QUI.Label(" 类别是 -> ", Style.Text.Small, 60);
                }
                QUI.EndVertical();
                QUI.BeginVertical(70);
                {
                    ...
                    QUI.BeginHorizontal();
                    {
                        ...
                        {
                            QUI.BeginVertical(120);
                            {
                                QUI.Space(SPACE_2);
                                QUI.SetGUIBackgroundColor(accentColorGreen);
                                newItemCategoryListIndex = EditorGUILayout.Popup(newItemCategoryListIndex, categoriesList.ToArray(), GUILayout.Width(120), GUILayout.Height(18));
                                QUI.ResetColors();
                                QUI.Space(-1);
                            }
                            QUI.EndVertical();
            ...
            QUI.BeginHorizontal(WIDTH_420);
            {
                if(QUI.GhostBar("新 Item 的默认设置", QColors.Color.Green, showNewItemSettings, WIDTH_420 - 100 - 4, 16))
                {
                    showNewItemSettings.target = !showNewItemSettings.target;
                }
                QUI.Space(SPACE_2);
                if(QUI.GhostButton("Reset", QColors.Color.Gray, 100))
                {
                    newItemSettings = new Pooly.Item();
            ...
            if(QUI.BeginFadeGroup(showNewItemSettings.faded))
            {
                ...
                QUI.EndHorizontal();
                QUI.Space(SPACE_4);
                DrawItemSettings(newItemSettings, WIDTH_420-10, showNewItemSettings, IconColor.Green, accentColorGreen);
                QUI.Space(SPACE_4);
            }
            QUI.EndFadeGroup();
            QUI.Space(SPACE_8);
        ...
        void DrawSearchArea()
        {
            QUI.BeginHorizontal(WIDTH_420);
            {
                ...
                QUI.Space(1);
                if(QUI.GhostButton(string.IsNullOrEmpty(searchPattern) ? "关闭所有打开了的 Items " : "清除搜索并打开", QColors.Color.Gray, 120, 16))
                {
        ...
        {
            if(QUI.GhostBar((item.prefab == null) ? "丢失 Prefab" : item.prefab.name,
            ...
            if(QUI.BeginFadeGroup(itemExpanded[item].faded))
            {
                ...
                QUI.BeginHorizontal(width);
                {
                    ...
                    if(item.prefab != null)
                    {
                        ...
                        Texture2D assetPreview = AssetPreview.GetAssetPreview(item.prefab.gameObject);
                        if(assetPreview == null)
                        {
                            QUI.Box(new GUIContent("Prefab"), QStyles.GetStyle(Style.SlicedButton.BlueSelected), 36, 36 * itemExpanded[item].faded);
                ...
                QUI.BeginHorizontal(width);
                {
                    ...
                    QUI.BeginVertical(width);
                    {
                        QUI.BeginHorizontal(width);
                        {
                            QUI.Label("克隆数量", Style.Text.Normal, 80);
                            QUI.Space(18);
                            if (item.prefab == null)
                            {
                                QUI.SetGUIBackgroundColor(QUI.IsProSkin ? QColors.Red.Color : QColors.RedLight.Color);
                            }
                            if((item.prefab == null && itemExpanded[item].faded > 0.1f))
                            {
                                EditorGUILayout.ObjectField(GUIContent.none, item.prefab, typeof(Transform), false, GUILayout.Width(width - SPACE_16 - 108 - 40));
                            }
                            if (item.prefab != null)
                            {
                                GameObject go =  EditorGUILayout.ObjectField(GUIContent.none, item.prefab.gameObject, typeof(GameObject), false, GUILayout.Width(width - SPACE_16 - 108 - 40)) as GameObject;
                                if (go !=null)
                                {
                                    item.prefab = go.transform;
                                }
                            }
                        ...
                        QUI.BeginHorizontal(width);
                        {
                            ...
                            {
                                if(QUI.GhostButton("改变 类别", QColors.Color.Gray, 130, 16 * itemExpanded[item].faded))
                                {
        ...
        void DrawCategoryOptionsButtons(string category)
        {
            QUI.BeginHorizontal(WIDTH_420);
            {
                ...
                if(string.IsNullOrEmpty(renameCategory) || !category.Equals(renameCategory)) //if the renameCategory is empty or if this is not the category we are renaming, we show the category option buttons
                {
                    ...
                    QUI.Space(SPACE_2);
                    if(QUI.GhostButton("修改 类别 名称", QColors.Color.Gray, (WIDTH_420 - SPACE_8 - SPACE_2 - SPACE_2) / 3, 20 * categoryExpanded[category].faded))
                    {
                    ...
                    QUI.Space(SPACE_2);
                    if(QUI.GhostButton("删除 类别", QColors.Color.Gray, (WIDTH_420 - SPACE_8 - SPACE_2 - SPACE_2) / 3, 20 * categoryExpanded[category].faded))
                    {
                        if(QUI.DisplayDialog("删除 类别",
                                                       "你确定要删除 '" + category + "' 这个类别? 这将会删除所有的 Items" +
                                                       "\n\n" +
                                                       "操作无法撤消！",
                                                       "Ok",
                                                       "Cancel"))
