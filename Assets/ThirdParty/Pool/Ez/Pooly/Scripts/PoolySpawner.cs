...
namespace Ez.Pooly
{
    ...
    [AddComponentMenu("我的组件/对象池/放在场景当对象产生器")]
    public class PoolySpawner : MonoBehaviour 
    {
        public void StartSpawn()                                 // 最简单的 Spawn ，没有延时，从一开始的地方产生
        {
        ...
        /// 带延时的产生
        /// </summary>
        /// <param name="spawnDelay">Delay before spawning starts.</param>
        ...
        public void StartSpawn(float spawnDelay, bool resetSpawnedCount = true, bool isResumeSpawn = false)
        {
            if (!AreSpawnerSettingsValid()) { return; }
            InternalStopSpawn(resetSpawnedCount);
            if (!isResumeSpawn) { ResetSpawnIndex(); }
            cSpawn = StartCoroutine(iStartSpawn(spawnDelay, isResumeSpawn));
        }



        /// <summary>
        /// 如果启用了spawnForever 或者spawnedCount小于spawnCount，则返回true
        /// </summary>
        ...
        /// 停止产生周期
        /// </summary>
        /// <param name="resetSpawnedCount">如果设置为false，它将充当PauseSpawn并且不会重置spawnedCount计数器</param>
        ...
        /// <summary>
        /// 如果至少有一个预制体被引用到 spawnPrefabs 列表，则返回true
        /// </summary>
        ...
        /// <summary>
        ///  如果至少有一个 坐标点 被引用到 spawnPrefabs 列表，则返回true
        /// </summary>
        ...
        /// <summary>
        /// 如果启用了spawnForever 或者 spawnedCount小于spawnCount，则在下一个设置位置生成预制件
        /// <para>这也增加了spawnedCount </para>
        /// <para>如果要在生成周期中手动触发下一个预制件的生成，此方法很有用</para>
        /// </summary>
        ...
        #region 三个 Bean :SpawnPrefab / SpawnPoint / SpawnPosition  
        [Serializable]
        ...
        void OnSpawned()
        {
            if (autoStart == AutoStart.OnSpawned)
            {
                StartSpawn(spawnStartDelay);
            }
        }
        ...
        {
            if (spawnPrefabs == null || spawnPrefabs.Count < 1)
            {
                Debug.LogWarning("[Pooly][PoolySpawner] Link at least one prefab, to spawn, for the '" + name + "' Pooly Spawner. Spawner disabled..."); return false;
            }
            if (autoStart != AutoStart.Never && spawnCount < 1)
            {
                Debug.LogWarning("[Pooly][PoolySpawner] The spawnCount should be greater than zero (0) for the '" + name + "' Pooly Spawner. Either increase the spawnCount, or set autoStart to Never. Spawner disabled..."); return false;
            }
        ...
        /// 内部方法停止生成循环
        /// </summary>
        /// <param name="resetSpawnedCount">true:暂停会记录下之前发射到第几个索引</param>
        /// <param name="shouldSendEvent">如果设置为true，则触发OnSpawnStopped事件</param>
        ...
        IEnumerator iStartSpawn(float startDelay, bool isResumeSpawn)
        {
            ...
            if (autoStart == AutoStart.OnSpawned && despawnWhenFinished)
            {
                Despawn();
            }
        }
