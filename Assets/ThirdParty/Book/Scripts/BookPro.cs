...
public class BookPro : MonoBehaviour
{
    ...
    public UnityEvent OnFlip;                        // 在任何页面翻转时调用


    public int CurrentPaper                         // 当前显示的页数
    {
    ...
    private const int MY_ADD_WIDTH = 150;
    ...
    void Awake()
    {
        t_ZiDong = transform.Find("ZiDongFang");
        mBgToggle = transform.parent.Find("Bg").GetComponent<DTToggle3_Fade>();
    ...
    void Start()
    {
        ...
        float pageWidth = BookPanel.rect.width / 2.0f+ MY_ADD_WIDTH;
        float pageHeight = BookPanel.rect.height+ MY_ADD_WIDTH;
        ...
        Shadow.rectTransform.sizeDelta = new Vector2(pageWidth+150, shadowPageHeight);
    ...
    {
        UpdateBook(Input.mousePosition);
    ...
    private Transform t_ZiDong;
    private DTToggle3_Fade mBgToggle;


    public void StartZiDongFang(float dalay,int toPage)   // 先第0页-> 到指定页数
    {
        mBgToggle.SetFistInit();
        CurrentPaper = 0;
        StartCoroutine(ZiDongFang(dalay,toPage));
    }


    IEnumerator ZiDongFang(float dalay, int toPage)  
    {
        interactable = false;
    
        yield return new WaitForSeconds(dalay);
        for (int i = 0; i < t_ZiDong.childCount; i++)
        {
            if (i == 0)
            {
                DragRightPageToPoint(t_ZiDong.GetChild(0).localPosition);
            }
            else
            {
                UpdateBook(t_ZiDong.GetChild(i).localPosition);
            }
    ...
    IEnumerator SetBg2Big()                              // 设置背景成大的那个
    {
        yield return new WaitForSeconds(0.4f);
        if ((mBgToggle.IsFirsting && CurrentPaper == 1)||(mBgToggle.IsThree&& CurrentPaper != papers.Length))
        {
            mBgToggle.Change2Two();
        }
    ...
    public void ReleasePage()                                        // 释放页（相当于把页拿起来后放手）
    {
        if (pageDragging)
        {
            pageDragging = false;
        ...
        if (CurrentPaper == 1|| mBgToggle.IsThree)
        {
            StartCoroutine(SetBg2Big());              // 改成大背景
        }
    ...
    public void UpdateBook(Vector3 position)                       // 一直调用
    {
        f = Vector3.Lerp(f, transformPointMousePosition(position), Time.deltaTime * 10);
        if (mode == FlipMode.RightToLeft)
            UpdateBookRTLToPoint(f);
        else
            UpdateBookLTRToPoint(f);
