...
public class BookProEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("更新页面顺序"))
        {
            (target as BookPro).UpdatePages();
        }
        if (GUILayout.Button("更新页面名称"))
        {
            if (EditorUtility.DisplayDialog("Warning!",
                "所有页面将根据其顺序重命名", "Ok", "Cancel"))
            {
                BookPro book = target as BookPro;
    ...
    private void DrawFlippingPapersRange()
    {
        ...
        EditorGUILayout.LabelField("翻转范围(即可限定翻页)", EditorStyles.boldLabel);
        EditorGUILayout.LabelField("第一张可翻的纸页是: " + " 第 " + book.StartFlippingPaper+" 页");
        EditorGUILayout.LabelField("最后张可翻的纸页是: " + " 第 " + book.EndFlippingPaper+" 页");
