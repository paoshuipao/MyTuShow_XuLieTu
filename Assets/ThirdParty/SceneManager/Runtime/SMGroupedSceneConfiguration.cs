...
/// <summary>
/// The scene configuration as used by <see cref="MySceneManagerOld"/>
/// </summary>
public class SMGroupedSceneConfiguration : SMSceneConfigurationBase
{
    ...
    /// </summary>
    public WorkflowActionEnum actionAfterGroup;
    ...
    public override bool IsValid(HashSet<string> validScenes)
    {
        ...
        if (actionAfterGroup == WorkflowActionEnum.LoadScreen)
        {
            if (String.IsNullOrEmpty(firstScreenAfterGroup))
            {
                Debug.LogWarning("In configuration '" + name +
                                 "' you configured that you want to load a screen after a group has finished. " +
        ...
        if (groups.Length == 0)
        {
            Debug.LogWarning("The scene configuration '" + name +
                             "' has no groups set up. Please add at least one group by " +
        ...
        {
            Debug.LogWarning("The scene configuration '" + name +
                             "' does not contain a level. To add a level select the scene " +
        ...
        for (int i = 1; i < groupOffset.Length; i++)
        {
            ...
            {
                Debug.LogWarning("The scene configuration '" + name +
                                 "' contains a group with no levels in it. To add a level " +
