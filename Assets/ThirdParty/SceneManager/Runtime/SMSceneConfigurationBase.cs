...
public abstract class SMSceneConfigurationBase : ScriptableObject
{
    ...
    public bool activeConfiguration = false;                     // 是否激活了这个配置文件
    ...
    public virtual bool IsValid(HashSet<string> validScenes)
    {
        ...
        {
            Debug.LogWarning("The scene configuration '" + name +
                             "' has no first screen. The first screen determines " +
                             "which unity scene is shown when the game launches. To set a first screen, open this scene configuration, " +
                             "then select a scene that should be the first screen and press the 'First Screen' button.",
                this);
            result = false;
        }
        ...
        {
            Debug.LogWarning("The scene configuration '" + name +
                             "' has not set up which scene should be loaded after the " +
                             "last level of the game has been played. To set this up, open this scene configuration, then select a scene " +
                             "that should be played after the last level and click the 'After last Level' button.",
                this);
            result = false;
        }
        ...
        foreach (string screen in screens)
        {
            ...
            {
                Debug.LogWarning("The scene configuration '" + name + "' references a unity scene named '" + screen +
                                 "', hoever this " +
        ...
        foreach (string level in levels)
        {
            ...
            {
                Debug.LogWarning("The scene configuration '" + name + "' references a unity scene named '" + level +
                                 "', hoever this " +
