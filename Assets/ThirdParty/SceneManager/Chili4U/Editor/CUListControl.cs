...
public class CUListControl
{
    ...
    private static AB_CUItemRenderer<object> defaultItemRenderer;
    ...
    public static CUListData SelectionList<T>(CUListData listData, IList<T> items, AB_CUItemRenderer<T> itemRenderer,
        params GUILayoutOption[] options)
    {
        return SelectionList(listData, items, itemRenderer, null, options);
    }

    public static CUListData SelectionList(CUListData listData, IList<object> items, string caption,
        params GUILayoutOption[] options)
    {
        return SelectionList(listData, items, defaultItemRenderer, caption, options);
    }

    public static CUListData SelectionList<T>(CUListData listData, IList<T> items, AB_CUItemRenderer<T> itemRenderer, string caption, params GUILayoutOption[] options)
    {
        ...
        if (currentEvent == EventType.DragUpdated || currentEvent == EventType.DragPerform ||
            currentEvent == EventType.DragExited)
        {
        ...
        foreach (T item in items)
        {
            ...
            switch (currentEvent)
            {
                ...
                case EventType.Repaint:
                    ...
                    {
                        if (state.dropType == EnumDropType.AtPosition)
                        {
                ...
                case EventType.DragPerform:
                    ...
                    {
                        bool canDropInto = listData.DropTarget.CanDrop(i, EnumDropType.IntoItem);
                        // check if this is a drop between
                        if (IsDropBefore(elementRect, mousePos, canDropInto) &&
                            listData.DropTarget.CanDrop(i, EnumDropType.AtPosition))
                        {
                            state.isDropTarget = true;
                            state.itemIndex = i;
                            state.dropType = EnumDropType.AtPosition;
                            if (currentEvent == EventType.DragPerform)
                            {
                                listData.DropTarget.AcceptDrop(i, EnumDropType.AtPosition);
                            }
                        }

                        // now check if this is a drop after
                        if (!state.isDropTarget && IsDropAfter(elementRect, mousePos, canDropInto) &&
                            listData.DropTarget.CanDrop(i + 1, EnumDropType.AtPosition))
                        {
                            state.isDropTarget = true;
                            state.itemIndex = i + 1;
                            state.dropType = EnumDropType.AtPosition;
                            if (currentEvent == EventType.DragPerform)
                            {
                                listData.DropTarget.AcceptDrop(i + 1, EnumDropType.AtPosition);
                            }
                        }
                        ...
                        if (!state.isDropTarget && canDropInto)
                        {
                            state.isDropTarget = true;
                            state.itemIndex = i;
                            state.dropType = EnumDropType.IntoItem;
                            if (currentEvent == EventType.DragPerform)
                            {
                                listData.DropTarget.AcceptDrop(i, EnumDropType.IntoItem);
                            }
                        }
        ...
        {
            case EventType.ContextClick:
                if (GUIUtility.hotControl != 0 || !scrollViewRect.Contains(Event.current.mousePosition) ||
                    !listData.IsContextMenuSupported)
                {
                    break;
                }
            ...
            case EventType.Repaint:
                ...
                {
                    if (state.dropType == EnumDropType.AtPosition && state.itemIndex == items.Count)
                    {
                        if (items.Count == 0)
                        {
                    ...
                    else if (state.dropType == EnumDropType.IntoContainer)
                    {
            ...
            case EventType.DragPerform:
                ...
                {
                    if (listData.DropTarget.CanDrop(items.Count, EnumDropType.AtPosition))
                    {
                        state.isDropTarget = true;
                        ...
                        if (currentEvent == EventType.DragPerform)
                        {
                            listData.DropTarget.AcceptDrop(items.Count, EnumDropType.AtPosition);
                        }
                    }
                    else if (listData.DropTarget.CanDrop(-1, EnumDropType.IntoContainer))
                    {
                        state.isDropTarget = true;
                        state.itemIndex = -1;
                        state.dropType = EnumDropType.IntoContainer;
                        if (currentEvent == EventType.DragPerform)
                        {
                            listData.DropTarget.AcceptDrop(-1, EnumDropType.IntoContainer);
                        }
                    }
                    ...
                    if (Event.current.keyCode != KeyCode.None)
                    {
                        ...
                        else
                        {
                            ...
                            if ((Event.current.keyCode == KeyCode.Return ||
                                 Event.current.keyCode == KeyCode.KeypadEnter) && listData.IsExecutionSupported)
                            {
                                if (listData.ExecutionListener.HandleExecution(listData.Selection))
                                {
                                    handled = true;
                            ...
                            {
                                if (listData.KeyListener.HandleKeyEvent(Event.current.keyCode, Event.current.character,
        ...
        if (scrollToItem != -1)
        {
            ...
            Rect viewport =
                new Rect(0, listData.scrollPosition.y, 100, scrollViewRect.height); // real width is not important
